...
module.exports = (req, res, next) => {
  try {
    ...
    if (!validator.isLength(req.body.name, { min: 2, max: undefined }))
      throw new Error("Name must be at least 2 characters");
    if (
      req.body.address &&
      !validator.isLength(req.body.address, { min: 2, max: undefined })
    )
      throw new Error("Address must be at least 2 characters");
    if (
      req.body.city &&
      !validator.isLength(req.body.city, { min: 2, max: undefined })
    )
      throw new Error("City must be at least 2 characters");
    if (
      req.body.state &&
      !validator.isLength(req.body.state, { min: 2, max: undefined })
    )
      throw new Error("State must be at least 2 characters");
    if (
      req.body.zipcode &&
      !validator.isLength(req.body.zipcode, { min: 5, max: undefined })
    )
      throw new Error("Zipcode must be at least 5 characters");
    if (
      req.body.country &&
      !validator.isLength(req.body.country, { min: 2, max: undefined })
    )
